    ...
    <PageLayout>
      ...
      <div style={{ position: "relative", height: 280, overflow: "hidden", marginBottom: 0 }}>
        ...
        <div style={{ position: "relative", maxWidth: 1280, margin: "0 auto", padding: "0 16px", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <h1 style={{ color: "#fff", fontSize: 36, fontWeight: 800, marginBottom: 8 }}>کالکشن‌ها</h1>
          <p style={{ color: "#aaa", fontSize: 15 }}>مجموعه‌ای از بهترین طلاها برای هر سلیقه</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "48px 16px" }}>
        ...
        <h2 style={{ color: "#fff", fontSize: 22, fontWeight: 700, marginBottom: 24 }}>کالکشن‌های ویژه</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 56 }} className="highlights-grid">
          {highlights.map((h, i) => (
            <Link key={i} href="/products" style={{ textDecoration: "none", display: "block", borderRadius: 12, overflow: "hidden", position: "relative", height: 240, border: "1px solid #2a2a2a", transition: "border-color 0.2s" }}
              ...
              <div style={{ position: "absolute", bottom: 0, right: 0, left: 0, padding: 20 }}>
                <h3 style={{ color: "#fff", fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{h.title}</h3>
                <p style={{ color: "#aaa", fontSize: 12, marginBottom: 10 }}>{h.desc}</p>
                <span style={{ color: "#d4af37", fontSize: 12, display: "inline-flex", alignItems: "center", gap: 4 }}>مشاهده <ChevronLeft size={12} /></span>
              </div>
            </Link>
          ))}
        </div>
        ...
        {/* Categories */}
        <h2 style={{ color: "#fff", fontSize: 22, fontWeight: 700, marginBottom: 24 }}>دسته‌بندی محصولات</h2>
        {loading ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} style={{ backgroundColor: "#1a1a1a", borderRadius: 12, height: 320, border: "1px solid #2a2a2a", opacity: 0.4 }} />
            ))}
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 20 }}>
            {categories.map((cat, i) => {
              ...
              return (
                <Link key={cat.id} href={`/products?category=${cat.id}`} style={{ textDecoration: "none", display: "block", borderRadius: 12, overflow: "hidden", position: "relative", height: 320, border: "1px solid #2a2a2a", transition: "border-color 0.2s, transform 0.2s" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "#d4af37"; (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)"; }}
                  ...
                  <div style={{ position: "absolute", bottom: 0, right: 0, left: 0, padding: "20px 16px" }}>
                    <span style={{ backgroundColor: "rgba(212,175,55,0.2)", border: "1px solid rgba(212,175,55,0.4)", color: "#d4af37", fontSize: 11, padding: "3px 10px", borderRadius: 20, marginBottom: 8, display: "inline-block" }}>
                      {cat.product_count} محصول
                    </span>
                    <h3 style={{ color: "#fff", fontSize: 20, fontWeight: 700, marginBottom: 4 }}>{cat.name}</h3>
                    {cat.description && <p style={{ color: "#aaa", fontSize: 12 }}>{cat.description}</p>}
                    <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 4, color: "#d4af37", fontSize: 13 }}>

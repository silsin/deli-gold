  ...
  return (
    <section style={{ maxWidth: "1280px", margin: "40px auto", padding: "0 16px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <Link href="/products" style={{ color: "var(--theme-accent)", fontSize: "13px", textDecoration: "none", display: "flex", alignItems: "center", gap: "4px" }}>
          <ChevronLeft size={14} /> مشاهده همه محصولات
        </Link>
        <h2 style={{ color: "var(--theme-text)", fontSize: "22px", fontWeight: "700" }}>محصولات محبوب</h2>
      </div>

      <div style={{ position: "relative" }}>
        <button onClick={() => setStart(s => Math.max(0, s - 1))} disabled={!canPrev} aria-label="قبلی"
          style={{ position: "absolute", right: "-20px", top: "50%", transform: "translateY(-50%)", backgroundColor: canPrev ? "color-mix(in srgb, var(--theme-accent) 15%, transparent)" : "rgba(50,50,50,0.5)", border: `1px solid ${canPrev ? "color-mix(in srgb, var(--theme-accent) 40%, transparent)" : "var(--theme-border)"}`, color: canPrev ? "var(--theme-accent)" : "var(--theme-text-muted)", width: "40px", height: "40px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: canPrev ? "pointer" : "default", zIndex: 10 }}>
          <ChevronRight size={18} />
        </button>
        <button onClick={() => setStart(s => Math.min(products.length - visible, s + 1))} disabled={!canNext} aria-label="بعدی"
          style={{ position: "absolute", left: "-20px", top: "50%", transform: "translateY(-50%)", backgroundColor: canNext ? "color-mix(in srgb, var(--theme-accent) 15%, transparent)" : "rgba(50,50,50,0.5)", border: `1px solid ${canNext ? "color-mix(in srgb, var(--theme-accent) 40%, transparent)" : "var(--theme-border)"}`, color: canNext ? "var(--theme-accent)" : "var(--theme-text-muted)", width: "40px", height: "40px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: canNext ? "pointer" : "default", zIndex: 10 }}>
          <ChevronLeft size={18} />
        </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "12px", overflow: "hidden" }}>
          {products.slice(start, start + visible).map((p, i) => {
            ...
            return (
              <div key={p.id} style={{ backgroundColor: "var(--theme-card)", border: "1px solid var(--theme-border)", borderRadius: "10px", overflow: "hidden", transition: "border-color 0.2s, transform 0.2s" }}
                ...
                <Link href={`/products/${p.slug}`} style={{ display: "block", textDecoration: "none", position: "relative" }}>
                  <div style={{ position: "relative", aspectRatio: "1/1" }}>
                    ...
                    <button onClick={e => { e.preventDefault(); toggleLike(p.id); }} aria-label="علاقه‌مندی"
                      style={{ position: "absolute", top: "8px", left: "8px", backgroundColor: "rgba(0,0,0,0.6)", border: "none", borderRadius: "50%", width: "32px", height: "32px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: isLiked ? "var(--theme-accent)" : "var(--theme-text-muted)" }}>
                ...
                <div style={{ padding: "12px" }}>
                  <Link href={`/products/${p.slug}`} style={{ textDecoration: "none" }}>
                    <h3 style={{ color: "var(--theme-text)", fontSize: "13px", fontWeight: "600", marginBottom: "6px" }}>{p.name}</h3>
                  </Link>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 4 }}>
                    <div>
                      <p style={{ color: "var(--theme-accent)", fontSize: "13px", fontWeight: "700" }}>
                        {finalPrice.toLocaleString("fa-IR")} <span style={{ color: "var(--theme-text-muted)", fontSize: "11px", fontWeight: "400" }}>تومان</span>
                      </p>
                      <p style={{ color: "var(--theme-text-muted)", fontSize: "11px" }}>وزن: {p.weight}گ · {p.karat} عیار</p>
                    </div>
                    <button
                      onClick={e => handleAdd(p, img, e)}
                      disabled={p.stock === 0}
                      title={p.stock === 0 ? "ناموجود" : "افزودن به سبد"}
                      style={{ width: 30, height: 30, flexShrink: 0, backgroundColor: isAdded ? "rgba(16,185,129,0.2)" : inCart ? "color-mix(in srgb, var(--theme-accent) 15%, transparent)" : "color-mix(in srgb, var(--theme-accent) 10%, transparent)", border: `1px solid ${isAdded ? "rgba(16,185,129,0.4)" : "color-mix(in srgb, var(--theme-accent) 30%, transparent)"}`, borderRadius: 6, cursor: p.stock === 0 ? "not-allowed" : "pointer", color: isAdded ? "#10b981" : "var(--theme-accent)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "6px", marginTop: "20px" }}>
          {Array.from({ length: products.length - visible + 1 }).map((_, i) => (
            <button key={i} onClick={() => setStart(i)} aria-label={`صفحه ${i + 1}`}
              style={{ width: i === start ? "20px" : "8px", height: "8px", borderRadius: "4px", backgroundColor: i === start ? "var(--theme-accent)" : "color-mix(in srgb, var(--theme-accent) 30%, transparent)", border: "none", cursor: "pointer", padding: 0, transition: "all 0.3s" }} />
          ))}
        </div>
      )}
    </section>

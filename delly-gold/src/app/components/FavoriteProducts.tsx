...
interface Product {
  ...
  weight: number; karat: number; images: string;
}
      ...
      <div style={{ position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "12px", overflow: "hidden" }}>
          {products.slice(start, start + visible).map((p, i) => {
            ...
            return (
              <div key={p.id} style={{ backgroundColor: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: "10px", overflow: "hidden", transition: "border-color 0.2s, transform 0.2s", cursor: "pointer" }}
                ...
                <Link href={`/products/${p.slug}`} style={{ display: "block", textDecoration: "none", position: "relative" }}>
                  <div style={{ position: "relative", aspectRatio: "1/1" }}>
                    <div style={{ width: "100%", height: "100%", backgroundImage: `url(${getImage(p, start + i)})`, backgroundSize: "cover", backgroundPosition: "center", minHeight: "160px" }} />
                    {/* Heart */}
                ...
                {/* Info */}
                <Link href={`/products/${p.slug}`} style={{ display: "block", textDecoration: "none", padding: "12px" }}>
                  ...
                  <p style={{ color: "#d4af37", fontSize: "13px", fontWeight: "700" }}>
                    {p.price.toLocaleString("fa-IR")} <span style={{ color: "#888", fontSize: "11px", fontWeight: "400" }}>تومان</span>
                  </p>
                  <p style={{ color: "#666", fontSize: "11px", marginTop: "2px" }}>وزن: {p.weight}گ · {p.karat} عیار</p>
                </Link>
              </div>
            );
          })}

  ...
  return (
    <section style={{ maxWidth: "1280px", margin: "40px auto", padding: "0 16px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: "16px", alignItems: "stretch" }} className="collections-wrapper">
        ...
        {/* Intro card */}
        <div style={{ backgroundColor: "var(--theme-card)", border: "1px solid var(--theme-border)", borderRadius: "12px", padding: "28px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <div>
            <div style={{ width: "40px", height: "3px", backgroundColor: "var(--theme-accent)", marginBottom: "16px" }} />
            <h2 style={{ color: "var(--theme-text)", fontSize: "24px", fontWeight: "700", marginBottom: "12px" }}>کالکشن‌ها</h2>
            <p style={{ color: "var(--theme-text-muted)", fontSize: "14px", lineHeight: "1.7" }}>
              دنبالی از هنر و طرح‌های شاخص<br />برای هر سلیقه
            </p>
            {categories.length > 0 && (
              <div style={{ marginTop: "16px", display: "flex", flexDirection: "column", gap: "6px" }}>
                {categories.map(c => (
                  <Link key={c.id} href={`/products?category=${c.id}`} style={{ color: "var(--theme-text-muted)", fontSize: "12px", textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid var(--theme-border)" }}
                    ...
                    <span>{c.name}</span>
                    <span style={{ color: "var(--theme-text-muted)", fontSize: "11px" }}>{c.product_count} محصول</span>
                  </Link>
                ))}
              </div>
            )}
          </div>
          <Link href="/collections" style={{ display: "inline-flex", alignItems: "center", gap: "6px", color: "var(--theme-accent)", fontSize: "13px", fontWeight: "600", textDecoration: "none", border: "1px solid color-mix(in srgb, var(--theme-accent) 40%, transparent)", padding: "8px 16px", borderRadius: "6px", backgroundColor: "color-mix(in srgb, var(--theme-accent) 10%, transparent)", width: "fit-content", marginTop: "20px" }}>
            <ChevronLeft size={14} />
            مشاهده همه
          </Link>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px" }} className="collections-sub">
          {(categories.length > 0 ? categories : Array.from({ length: 4 }, (_, i) => ({ id: String(i), name: ["گردنبند", "انگشتر", "دستبند", "گوشواره"][i], slug: ["necklaces", "rings", "bracelets", "earrings"][i], product_count: 0 }))).map((col, i) => (
            <Link key={col.id} href={categories.length > 0 ? `/products?category=${col.id}` : "/collections"} style={{ textDecoration: "none", display: "block", borderRadius: "10px", overflow: "hidden", position: "relative", aspectRatio: "3/4", border: "1px solid var(--theme-border)", transition: "border-color 0.2s, transform 0.2s" }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--theme-accent)"; (e.currentTarget as HTMLElement).style.transform = "scale(1.02)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--theme-border)"; (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}>
              ...
              <div style={{ position: "absolute", bottom: "12px", right: "12px", left: "12px" }}>
                <p style={{ color: "var(--theme-text)", fontSize: "13px", fontWeight: "600", marginBottom: "2px" }}>{col.name}</p>
                {col.product_count > 0 && <p style={{ color: "var(--theme-accent)", fontSize: "11px" }}>{col.product_count} محصول</p>}
              </div>
            </Link>
          ))}
        </div>
